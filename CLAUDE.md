...
**The Neon connection pool is *not* the bottleneck — that hypothesis was measured and disproved.** A real staging (Vercel lambdas + a Neon branch, see `k6/README.md`) took 200 concurrent VUs and ~90 writes/s with **zero 5xx and zero timeouts**: the serverless driver over WebSocket/443 doesn't hold a live TCP connection per lambda. Deployed write throughput (~90/s) is in fact *higher* than local (~70/s); the only thing the cloud makes worse is latency, a uniform ×5 (19 → 231 ms on reads) from network and lambda start-up. Don't assume the cloud scales worse than your laptop — measure it.
